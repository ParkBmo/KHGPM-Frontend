...
export default {
    ...
    requestCreateProductToSpring ({}, payload) {
        ...
        const { productName, content, writer, price, image } = payload
        return axios.post('http://localhost:7777/product/register',
            { productName, content, writer, price, image })
            .then(() => {
                alert('상품 등록 성공!')
            })
            .catch(() => {
    ...
    requestProductModifyToSpring ({}, payload) {
        const { productName, content, productId, writer, price, image } = payload
